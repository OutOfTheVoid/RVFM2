...
impl CommandList {
    ...
    pub fn read_u16(&self, offset: u32) -> Option<u16> {
        if ((offset + 1) as usize) < self.data.len() {
            Some(
                (self.data[offset as usize + 0] as u16) << 0 |
                (self.data[offset as usize + 1] as u16) << 8
            )
        } else {
            None
        }
    ...
    pub fn read_u32(&self, offset: u32) -> Option<u32> {
        if ((offset + 3) as usize) < self.data.len() {
            Some(
                (self.data[offset as usize + 0] as u32) << 0 |
                (self.data[offset as usize + 1] as u32) << 8 |
                (self.data[offset as usize + 2] as u32) << 16 |
                (self.data[offset as usize + 3] as u32) << 24
            )
        } else {
            None
        }
