...
impl Core {
    ...
    fn draw_blended_rect_rgba<const N_SRC: usize, const N_DST: usize, F_SRC_READ: Fn(&[u8; N_SRC]) -> [f32; 4], F_DST_READ: Fn(&[u8; N_DST])-> [f32; 4], F_DST_WRITE: Fn(&mut [u8; N_DST], [f32; 4])>(&mut self, src_tex: u8, dst_tex: u8, src_x: u16, src_y: u16, dst_x: u16, dst_y: u16, width: u16, height: u16, color_blend_op: ColorBlendOp, alpha_blend_op: AlphaBlendOp, f_src_read: &F_SRC_READ, f_dst_read: &F_DST_READ, f_dst_write: &F_DST_WRITE)
